"""
OASIS Classroom Simulation Script

Reads parameters from a simulation config file and executes a classroom
environment simulation using the Reddit OASIS platform (threaded discussion).
The Reddit platform is used because classroom discussion is inherently
threaded — a student posts a question; peers and instructors reply.
...
Usage:
    python run_classroom_simulation.py --config /path/to/simulation_config.json
    python run_classroom_simulation.py --config /path/to/simulation_config.json --no-wait
"""
...
class IPCHandler:
    """Handles IPC commands for the classroom environment."""
    ...
    def _get_interview_result(self, agent_id: int) -> Dict[str, Any]:
        """Query the classroom DB for the most recent interview response for *agent_id*."""
        db_path = os.path.join(self.simulation_dir, "classroom_simulation.db")
...
class ClassroomSimulationRunner:
    """Runs a classroom environment simulation using the Reddit OASIS platform."""

    # Reddit action types available to classroom agents.
    # The classroom framing is embedded in each agent's persona — the underlying
    # OASIS platform actions (post, comment, like, search …) map naturally to
    # classroom behaviours (submit work, respond to peers, search resources …).
    AVAILABLE_ACTIONS = [
    ...
    def _get_profile_path(self) -> str:
        return os.path.join(self.simulation_dir, "classroom_profiles.json")

    def _get_db_path(self) -> str:
        return os.path.join(self.simulation_dir, "classroom_simulation.db")
    ...
    def _get_active_agents_for_round(
        ...
        peak_hours = time_config.get("peak_hours", [9, 10, 11, 13, 14])
        off_peak_hours = time_config.get("off_peak_hours", list(range(0, 7)) + list(range(16, 24)))
    ...
    async def run(self, max_rounds: int = None):
        ...
        """
        print("=" * 60)
        print("OASIS Classroom Simulation")
        ...
        time_config = self.config.get("time_config", {})
        total_hours = time_config.get("total_simulation_hours", 40)
        ...
        print("Creating OASIS environment (Reddit platform)...")
        self.env = oasis.make(
        ...
        # Post initial seed content (e.g. instructor posts the assignment brief)
        event_config = self.config.get("event_config", {})
        initial_posts = event_config.get("initial_posts", [])

        if initial_posts:
            print(f"Posting initial content ({len(initial_posts)} seed posts)...")
            initial_actions = {}
            ...
            if initial_actions:
                ...
                print(f"  Posted {len(initial_actions)} seed posts.")
...
async def main():
    parser = argparse.ArgumentParser(description='OASIS Classroom Simulation')
    parser.add_argument(
        '--config',
        type=str,
        required=True,
